...
MCW_REPO := https://github.com/efabless/caravel_mgmt_soc_gf180mcu
MCW_TAG := $(MPW_TAG)
...
GF_CARAVEL=$(shell dirname $(PWD))
verify_command="source ~/.bashrc && cd ${GF_CARAVEL}/caravel/verilog/dv/caravel/mgmt_soc/$* && export SIM=${SIM} && make"
dv_base_dependencies=simenv
docker_run_verify=\
	docker run -v ${GF_CARAVEL}:${GF_CARAVEL} -v ${PDK_ROOT}:${PDK_ROOT} \
		-v ${CARAVEL_ROOT}:${CARAVEL_ROOT} \
		-e GF_CARAVEL=${GF_CARAVEL} -e PDK_ROOT=${PDK_ROOT} \
		-e CARAVEL_ROOT=${CARAVEL_ROOT} \
		-e TOOLS=/foss/tools/riscv-gnu-toolchain-rv32i/217e7f3debe424d61374d31e33a091a630535937 \
		-e PDK=$(PDK) \
		-e MCW_ROOT=$(GF_CARAVEL)/mgmt_soc_litex \
