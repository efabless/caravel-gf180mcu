...
def run_sta (
    ...
    pt_command = f"source /tools/bashrc_snps; pt_shell -f {PT_tcl} -output_log_file {log_dir}/{design}-{proc_corner}-{rc_corner}-sta.log"
    os.system(pt_command)
    # Check if there are any violations
    sta_pass=search_viol(f"{output_dir}/reports/{proc_corner}{proc_corner}/{design}.{rc_corner}-global.rpt")
...
def search_viol(
    ...
    rc_corner = report_path[-14:-11]
    proc_corner = os.path.basename(os.path.dirname(report_path))[0]
    log_path = report_path.replace(f"/reports/{proc_corner}{proc_corner}/", "/logs/")
    log_path = log_path.replace(f".{rc_corner}-global.rpt", f"-{proc_corner}-{rc_corner}-sta.log")
    with open(log_path, 'r') as report:
        ...
        if "Could not auto-link design" in data:
            ...
            return "no cons"
    log_path = log_path.replace(f"{proc_corner}-{rc_corner}-sta", f"{rc_corner}-parasitics")
    with open(log_path, 'r') as report:
        data = report.read()
