...
def run_sta(
    ...
    print(
        f"PrimeTime STA run for design: {design} at process corner {proc_corner} and RC corner {rc_corner}"
    )
    ...
    pt_command = f"pt_shell -f {PT_tcl} -output_log_file {log_dir}/{proc_corner}-{rc_corner}-sta.log"
    os.system(pt_command)

    log = open(f"{log_dir}/{proc_corner}-{rc_corner}-sta.log", "a")
    ...
    sta_pass = search_viol(
        f"{output_dir}/reports/{proc_corner}{proc_corner}/{rc_corner}-global.rpt",
        f"{log_dir}/{proc_corner}-{rc_corner}-sta.log",
    )
    if sta_pass == "pass":
        ...
        print(f"Linking failed. check log: {log_dir}/{proc_corner}-{rc_corner}-sta.log")
        log.write(
            f"Linking failed. check log: {log_dir}/{proc_corner}-{rc_corner}-sta.log"
        )
    else:
        if sta_pass == "max_tran_cap":
            ...
            log.write(f"STA run Passed!\n")
            print(
                f"There are max_transition and max_capacitance violations. check report: {output_dir}/reports/{proc_corner}{proc_corner}/{rc_corner}-all_viol.rpt"
            )
            log.write(
                f"There are max_transition and max_capacitance violations. check report: {output_dir}/reports/{proc_corner}{proc_corner}/{rc_corner}-all_viol.rpt"
            )
            ...
            log.write(f"STA run Passed!\n")
            print(
                f"There are max_transition violations. check report: {output_dir}/reports/{proc_corner}{proc_corner}/{rc_corner}-all_viol.rpt"
            )
            log.write(
                f"There are max_transition violations. check report: {output_dir}/reports/{proc_corner}{proc_corner}/{rc_corner}-all_viol.rpt"
            )
            ...
            log.write(f"STA run Passed!\n")
            print(
                f"There are max_capacitance violations. check report: {output_dir}/reports/{proc_corner}{proc_corner}/{rc_corner}-all_viol.rpt"
            )
            log.write(
                f"There are max_capacitance violations. check report: {output_dir}/reports/{proc_corner}{proc_corner}/{rc_corner}-all_viol.rpt"
            )
            ...
            print(
                f"There are other violations. check report: {output_dir}/reports/{proc_corner}{proc_corner}/{rc_corner}-all_viol.rpt"
            )
            log.write(
                f"There are other violations. check report: {output_dir}/reports/{proc_corner}{proc_corner}/{rc_corner}-all_viol.rpt"
            )
            ...
            if sta_pass == "setup":
                print(
                    f"There are setup violations. check report: {output_dir}/reports/{proc_corner}{proc_corner}/{rc_corner}-global.rpt"
                )
                log.write(
                    f"There are setup violations. check report: {output_dir}/reports/{proc_corner}{proc_corner}/{rc_corner}-global.rpt"
                )
                ...
                print(
                    f"There are hold violations. check report: {output_dir}/reports/{proc_corner}{proc_corner}/{rc_corner}-global.rpt"
                )
                log.write(
                    f"There are hold violations. check report: {output_dir}/reports/{proc_corner}{proc_corner}/{rc_corner}-global.rpt"
                )
                ...
                print(
                    f"Reading constraints SDC failed. check log: {log_dir}/{proc_corner}-{rc_corner}-sta.log"
                )
                log.write(
                    f"Reading constraints SDC failed. check log: {log_dir}/{proc_corner}-{rc_corner}-sta.log"
                )
...
def check_env_vars():
    ...
    if pdk_root is None:
        raise FileNotFoundError("Please export PDK_ROOT to the PDK path")
    if pdk is None:
        raise FileNotFoundError("Please export PDK to either sky130A or sky130B")
    if caravel_root is None:
        raise FileNotFoundError("Please export CARAVEL_ROOT to the Caravel repo path")
    if mcw_root is None:
        raise FileNotFoundError(
...
def search_viol(report_path: str, log_path: str):
...
def find_missing_spefs(log_path: str):
    missing_spefs = 0
    spefs = []
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run STA using PrimeTime")
    parser.add_argument("-d", "--design", help="design name", required=True)
    parser.add_argument("-o", "--output_dir", help="output directory", required=True)
    parser.add_argument("-l", "--logs_dir", help="log directory", required=True)
    parser.add_argument("-r", "--root_dir", help="design root directory", required=True)
    parser.add_argument(
        "-rc",
        "--rc_corner",
    ...
    parser.add_argument(
        "-si",
        "--si_analysis",
        help="Specify to enable PrimeTime signal integrity (SI) analysis",
        action="store_true",
    )
    ...
    if args.si_analysis:
        os.environ["SI_ANALYSIS"] = "1"
    else:
        os.environ["SI_ANALYSIS"] = "0"
